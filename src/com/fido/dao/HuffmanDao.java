...
public class HuffmanDao {
	 ...
	 //读取要编码的文件,生成编码后的文件,返回是否成功
      public void encoding(String path,String newPath) throws IOException{
    	     HashMap<Character,Integer> hashMap=ArticleUtils.readAndCount(path).getMap();//获得字符和对应频数
...
//    	     
             ...
             try{
    			 ...
    			 while((aa=reader.read())!=-1){//未到文件末尾
    				          ...
    				          if(bufferStr.length()%8==0){//满8的倍数位了
    				        	     ...
    				        	     System.out.println("前："+bufferStr.length());
    				        	     bufferStr.delete(0, bufferStr.length());	
    				        	     System.out.println("后："+bufferStr.length());
    				          }	
    				          System.out.println("中间："+bufferStr.length());
    					}
                  System.out.println("剩下的："+bufferStr.length());
    			 //那么我最后剩下的01串不够80位,还要分是否为8的倍数的情况
        ...
        public void decoding(String path,String newPath) throws IOException, ClassNotFoundException{
        	  ...
        	  int aa;//代表读到的整数
        	  ObjectInputStream objectIn=new ObjectInputStream(fileIn);//用来度编码表的
        	  HashMap<Character,String> encodingMap=(HashMap<Character, String>) objectIn.readObject();//得到编码表
                    ...
                    if(fileIn.available()<=1){ //读到最后两个整数(因为还要考虑到补零个数的情况，补零个数不会超过7个
                       if(fileIn.available()==0){
             		    ...
             		    for(int i=0;i<buffer.length();i++){
             		    	   ...
             		    	   System.out.println("查找到:"+ss.toString());
             		    ...
             		    System.out.println("ss的长度"+current);
             		    ss.delete(0, ss.length());
